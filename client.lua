--[[
    j0k3r-pomade :: Client Logic (v1.2.0)
...
local function L(key)
    ...
    local tbl  = Config.Locales[lang] or Config.Locales['en']
...
local function notify(text)
    -- VORP "left" notification
    local ok = pcall(function()
        local Core = exports.vorp_core:GetCore()
        Core.NotifyLeft(L('title'), text, 'menu_textures', 'menu_icon_dot', 4000, 'COLOR_PURE_WHITE')
    end)
    if not ok then dprint('NotifyLeft failed (vorp_core not ready?)') end
end

-- ---------------------------------------------------------------------
-- The heart of the script: trigger the in-game pomade interaction,
-- then apply the slick-hair material to the current hair component.
-- ---------------------------------------------------------------------

RegisterNetEvent('j0k3r-pomade:applyPomade', function(_itemName)
    local playerPed = PlayerPedId()
    dprint('Pomade event received, starting interaction.')

    -- _IS_METAPED_USING_COMPONENT(ped, componentHash)
    -- 0x9925C067 == the "hat" category hash. If the player is wearing
    -- one, we pick the WITH_HAT variant so the game removes and puts
    -- the hat back on its own.
    local hasHat = Citizen.InvokeNative(0xFB4891BD7578CDC1, playerPed, tonumber(0x9925C067))
    ...
    -- The interaction animation runs for ~2.5s before the visual
    -- state of the hair is ready to be swapped.
    Wait(2500)

    -- _UPDATE_SHOP_ITEM_WEARABLE_STATE(ped, componentHash, stateHash, p3, p4, p5)
    -- This is what flips the hair material from "dry" to "pomaded".
    Citizen.InvokeNative(
        0x66B957AAC2EAAEAB,
        playerPed,
        GetCurrentPedComponent(playerPed, 'hair'),
        GetHashKey('POMADE'),
        0, true, 1
    )

    -- _UPDATE_PED_VARIATION(ped, p1, p2, p3, p4, p5)
    -- Refreshes the ped so the new hair material renders right away.
    Citizen.InvokeNative(
        0xCC8CA3E88256E58F,
        playerPed,
    ...
    notify(L('success'))
    dprint('Pomade applied successfully.')
end)

-- ---------------------------------------------------------------------
-- Test command: /pomadetest
-- Lets you verify the client side works without going through the
-- server's inventory hook. Run it from the F8 console.
-- ---------------------------------------------------------------------

RegisterCommand('pomadetest', function()
    dprint('/pomadetest invoked.')
    TriggerEvent('j0k3r-pomade:applyPomade', 'pomade')
end, false)

function GetCurrentPedComponent(ped, category)
    local componentsCount = GetNumComponentsInPed(ped)
    if not componentsCount then return 0 end
...
-- ---------------------------------------------------------------------
--   DataView library by gottfriedleibniz, used by the helpers above
--   to allocate the byte buffer that the shop-component natives
--   expect as out-parameters.
--
...
for label, datatype in pairs(DataView.Types) do
    DataStream[label] = function(self, endian, align)
        ...
        if not _ib(o, self.view.length, datatype) then return nil end
